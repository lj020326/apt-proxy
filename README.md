...
For clients that must keep end-to-end TLS (for example `Acquire::https::Proxy`), apt-proxy accepts HTTP `CONNECT` and opens a bidirectional tunnel.  Traffic is
encrypted between client and origin, so the proxy cannot inspect or cache it.

> **Security Note:** To prevent unauthorized tunneling or accidental package installations from untrusted hosts, `CONNECT` targets must be explicitly permitted in your `passthrough` allowlist. Unlisted hosts will receive an `HTTP 403 Forbidden` response.
...
use `CONNECT` only when tunneling is required for an allowlisted passthrough host.
